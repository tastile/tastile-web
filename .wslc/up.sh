...
if [[ "${TASTILE_INFISICAL_INJECTED:-}" != "1" ]]; then
  ...
  exec bun "$REPO_ROOT/scripts/run-with-infisical.mts" dev -- bash "$REPO_ROOT/.wslc/up.sh" "$@"
fi
...
if ! wslc images 2>/dev/null | awk 'NR>1 && $1 != "<none>" {print $1 ":" $2}' | grep -q "^$IMAGE:\$\|^$IMAGE:latest\$"; then
  echo "ERROR: image '$IMAGE' not found. Run .wslc/build.sh first."
  exit 1
fi
...
wslc run -d --name "$CONTAINER" --network "$NETWORK" \
  ...
  -e "TASTILE_RUST_API_URL=$TASTILE_RUST_API_URL" \
