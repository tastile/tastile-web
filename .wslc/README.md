...
# in this repo:
./.wslc/build.sh                # builds the tastile-web image (local; never pushed)
./.wslc/up.sh                   # starts the web container on host port 3000
./.wslc/status.sh               # shows container / image / network state
./.wslc/down.sh                 # stops + removes the container (image kept)
```
