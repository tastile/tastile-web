...
test("debug at012", async ({ page }) => {
  await cleanDb();
  const c = await page.request.post(V1_BASE + "/v1/tiles", { headers: { "content-type": "application/json", "x-owner-id": OWNER, "x-actor-id": ACTOR }, data: { idempotency_key: uuidv7like(), payload: { kind: 0, title: "AT-012 " + Date.now(), description: null, color: "#0ea5e9", icon: "check", external_id: null, plan_role: 0 } } });
  ...
  // Two appends: same layer (0=Recurring), same rank (0), different instant.
  ...
  const items = (await t.json()) as Array<{ placement_id: string; span: { start: string; end: string }; resolution: { state: number; violations: unknown[] } }>;
