...
test("debug at012b", async ({ page }) => {
  await cleanDb();
  const c = await page.request.post(V1_BASE + "/v1/tiles", { headers: { "content-type": "application/json", "x-owner-id": OWNER, "x-actor-id": ACTOR }, data: { idempotency_key: uuidv7like(), payload: { kind: 0, title: "AT-012b " + Date.now(), description: null, color: "#0ea5e9", icon: "check", external_id: null, plan_role: 0 } } });
  ...
  // Second append
  const r2 = await page.request.post(V1_BASE + "/v1/placements/" + placementId + "/changes", { headers: { "content-type": "application/json", "x-owner-id": OWNER, "x-actor-id": ACTOR }, data: { idempotency_key: uuidv7like(), payload: { placement_id: placementId, changeset: { id: uuidv7like(), owner_id: OWNER, target: { Placement: placementId }, layer: 0, rank: 0, changes: [{ id: uuidv7like(), key: { group: 5, item: null, part: 0 }, kind: 2, value: { Instant: "2026-07-01T09:40:00.000Z" }, merge: 0, source: 2, source_ref: null, rank: 0 }], activation: { when: null, until: null }, revoked: null, source: 2, source_ref: null, created_at: "2026-07-01T00:00:00Z", created_by: { at: "2026-07-01T00:00:00Z", actor: ACTOR, actor_kind: 0, command_id: uuidv7like() } } } } });
