...
test.describe("v1 - AT-021 materialize idempotency", () => {
  ...
  test("re-materializing the same (recurring, frame-rule, range) does not duplicate placements", async ({ request }) => {
    ...
    const recurringId = (await createRes.json()).aggregate.id;

    const ruleRes = await postV1(request, "/v1/recurring/" + recurringId + "/frame-rules", {
      idempotency_key: uuidv7like(),
      payload: {
    ...
    const first = await postV1(request, "/v1/recurring/" + recurringId + "/frame-rules/" + fruid + "/materialize", matPayload);
    ...
    const second = await postV1(request, "/v1/recurring/" + recurringId + "/frame-rules/" + fruid + "/materialize", matPayload2);
    expect(second.status()).toBeLessThan(300);

    const countRes = execFileSync("docker", [
      "exec", "-i", "tastile-core-db-1",
      "psql", "-U", "tastile", "-d", "tastile_db", "-At",
