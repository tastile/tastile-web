...
export function FlowSequencePanel({
	...
	const update = (id: string, patch: Partial<FlowSequence>) =>
		setFlows(
			flows.map((flow) => (flow.id === id ? { ...flow, ...patch } : flow)),
		);
			...
			<FormRow icon={<ListOrdered className="size-4" aria-hidden />}>
				<div className="flex flex-col gap-1">
					<span className="text-xs font-medium">
						{t("quickCreate.panel.flow.heading")}
					</span>
			...
			{flows.map((flow, flowIndex) => (
				...
				>
					<Group justify="space-between">
						<Text fw={600} size="xs">
							{t("quickCreate.panel.flow.flowIndexLabel", {
								index: flowIndex + 1,
							})}
						</Text>
						<ActionIcon
							...
							onClick={() =>
								setFlows(flows.filter((item) => item.id !== flow.id))
							}
						>
					...
					>
						<div className="flex flex-col gap-1">
							<span className="text-xs font-medium">
								{t("quickCreate.panel.flow.observesLabel")}
							</span>
							<MultiSelect
								data={signalOptions}
								value={flow.observes}
								onChange={(value) =>
									update(flow.id, {
										observes: value as FlowSequence["observes"],
									})
								}
							/>
						</div>
						...
						>
							<div className="flex flex-col gap-1">
								<div className="flex items-center justify-between">
									<span className="text-xs font-medium">
										{t("quickCreate.panel.flow.flowConditionLabel")}
									</span>
									<Button
										size="compact-xs"
										variant="outline"
					...
					{flow.candidateWhen ? (
						...
						>
							<div className="flex flex-col gap-1">
								<div className="flex items-center justify-between">
									<span className="text-xs font-medium">
										{t("quickCreate.panel.flow.candidateConditionLabel")}
									</span>
									<Button
										size="compact-xs"
										variant="outline"
								...
								<ConditionEditor
									...
									onChange={(candidateWhen) =>
										update(flow.id, { candidateWhen })
									}
									t={t}
									tileOptions={tileOptions}
									taskOptions={taskOptions}
						...
						<Button
							...
							variant="outline"
							onClick={() =>
								update(flow.id, { candidateWhen: initialCondition() })
							}
						>
					...
					>
						<div className="flex flex-col gap-1">
							<span className="text-xs font-medium">
								{t("quickCreate.panel.flow.gapRankLabel")}
							</span>
							<Group grow>
								<NumberInput
									...
									onChange={(value) =>
										update(flow.id, {
											minimumGapMs: (Number(value) || 0) * 60_000,
										})
									}
								/>
								<NumberInput
									...
									value={flow.rank}
									onChange={(value) =>
										update(flow.id, { rank: Number(value) || 0 })
									}
								/>
							</Group>
						</div>
					...
					>
						<div className="flex flex-col gap-1">
							<span className="text-xs font-medium">
								{t("quickCreate.panel.flow.cycleSettingsLabel")}
							</span>
							<Group grow>
								<Switch
									size="xs"
									...
									checked={flow.cycle}
									onChange={(e) =>
										update(flow.id, { cycle: e.currentTarget.checked })
									}
								...
								<Switch
									...
									onChange={(e) =>
										update(flow.id, {
											resetOnInterrupt: e.currentTarget.checked,
										})
									}
					...
					{flow.cycle && (
						<Text size="xs" c="dimmed">
							{t("quickCreate.panel.flow.cycleSummary", {
								...
								avg: Math.round(
									flow.steps.reduce(
										(s, st) => s + st.waitBeforeMs + st.emitDurationMs,
										0,
									) /
										flow.steps.length /
										60000,
					...
					{flow.steps.map((step, stepIndex) => (
						<Group key={step.id} grow align="end">
							<NumberInput
								label={t("quickCreate.panel.flow.stepWaitLabel", {
									index: stepIndex + 1,
								})}
								min={0}
									...
									update(flow.id, {
										steps: flow.steps.map((item) =>
											item.id === step.id
												? {
														...item,
														waitBeforeMs: (Number(value) || 0) * 60_000,
													}
												: item,
										),
									})
									...
									update(flow.id, {
										steps: flow.steps.map((item) =>
											item.id === step.id
												? {
														...item,
														emitDurationMs: (Number(value) || 1) * 60_000,
													}
												: item,
										),
									})
