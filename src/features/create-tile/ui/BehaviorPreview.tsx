...
function RepeatPreview({
	...
	const getRepeatLabel = () => {
		switch (recurring.repeatMode) {
			...
			case "weekly": {
				const days = weekdayLabelsFor(locale).reduce<string>(
					(acc, label, i) => {
	...
	return (
		<div className="space-y-1.5">
			<div className="flex items-center gap-2 text-xs text-foreground-muted">
				<Repeat size={12} className="shrink-0" aria-hidden="true" />
...
function SourcePreview({ source, t }: SourcePreviewProps) {
	...
	return (
		<div className="space-y-1.5">
			<div className="flex items-center gap-2 text-xs text-foreground-muted">
				<SlidersHorizontal size={12} className="shrink-0" aria-hidden="true" />
