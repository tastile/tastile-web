...
export function QuickCreate() {
	...
	const [_memoExpanded, setMemoExpanded] = useState(
		meta.memo.trim().length > 0,
	);
	...
	const setSubmitBlockedReasonFromStore = useQuickCreateStore(
	...
	const canSubmit =
		spanOrderValid && durationValid && taskOrderValid && !submitBlocked;

	// --- field-level error sync ---
	useEffect(() => {
		const errors = new Map<string, string>();
		if (!spanOrderValid)
			errors.set("time.span", t("quickCreate.invalidTemporalOrder"));
		if (!durationValid)
			errors.set("time.durationMinMax", t("quickCreate.invalidDurationRange"));
				...
				<Stack gap={0} className="flex-1 overflow-y-auto">
					...
					<div className="px-4 py-3">
						<FormRow
							...
							trailing={
								time.whenMode !== "none" &&
								(time.span.start || time.span.end || time.referenceId) ? (
									<CloseButton
										size="xs"
										onClick={() => {
								...
								<span className="truncate text-foreground">
									{recurring.repeatMode === "once" ? (
										...
										<>
											...
											{recurring.repeatMode === "weekly" &&
												recurring.weekdayMask > 0 && (
													<span className="ml-1 text-foreground-muted">
														{weekdayLabelsFor(locale).reduce<string>(
															(acc, label, i) => {
											...
											{recurring.repeatMode !== "condition" &&
												recurring.endDate && (
													<span className="ml-1 text-foreground-muted">
				...
				<>
					...
					>
						<RelationPanel
							...
							setRelations={(relations) =>
								setField("source.relations", relations)
							}
						/>
					</SubPanelShell>
					...
					>
						<FlowSequencePanel
							...
							setFlows={(flowSequences) =>
								setField("source.flowSequences", flowSequences)
							}
							t={t}
					...
					>
						<PlacementRulesPanel
							...
							setRules={(placementRules) =>
								setField("plan.planning.placementRules", placementRules)
							}
							t={t}
							tileOptions={tilePickerData}
							taskOptions={taskPickerData}
