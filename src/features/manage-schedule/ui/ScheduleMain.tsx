	...
	const filteredTiles = (() => {
		if (view === "upcoming") {
			return tiles
				...
				.sort((a, b) => {
					return (
						new Date(a.temporal.due_at).getTime() -
						new Date(b.temporal.due_at).getTime()
					);
				});
		}
		return tiles;
				...
				<span className="font-mono text-caption text-foreground-lighter">
					{view === "recurring"
						? recurring.loading
							...
							: t("panels.schedule.templatesFound", {
									count: recurring.templates.length,
								})
						: view === "placements"
							? placementsLoading
								? t("panels.schedule.loading")
			...
			<div className="mt-4">
				...
				{view === "recurring" &&
					...
					recurring.templates.length === 0 && (
						...
						>
							<p className="text-sm">
								{t("panels.schedule.noRecurringTemplatesBody")}
							</p>
						</Alert>
					)}
				{view !== "recurring" &&
											...
											>
												<div className="min-w-0">
													<div className="font-mono text-xs text-foreground-subtle">
														{t("panels.schedule.workLabel")}{" "}
														{p.work_tile_id.slice(0, 8)}
													</div>
													<div className="font-mono text-xs text-foreground-subtle">
														{t("panels.schedule.blockLabel")}{" "}
														{p.time_tile_id.slice(0, 8)}
													</div>
												</div>
												<span className="rounded bg-surface-3/50 border border-border px-1.5 py-0.5 text-caption font-semibold uppercase tracking-wide text-foreground-subtle shrink-0">
													{t("panels.schedule.placedMinutes", {
														minutes: p.planned_minutes,
													})}
												</span>
											</div>
										))}
...
function describeGenerator(
	...
	if (typeof generator.step_min === "number") {
		return t("panels.schedule.generatorStepInterval", {
			step: generator.step_min,
		});
	}
