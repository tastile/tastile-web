...
function parseRangeParam(raw: string): { val: number; unit: RangeUnit } {
	...
	if (!Number.isNaN(num) && VALID_RANGE_UNITS.includes(unit))
		return { val: num, unit };
...
function parseGranularityParam(raw: string): {
	...
	if (minPart) {
		const minutes = Number.parseInt(
			minPart.replace("min_", "").replace("m", ""),
			10,
		);
...
export function TasksSidePanel() {
	...
	const [rangeUnit, setRangeUnit] = useState<"d" | "w" | "m">(
		initialRange.unit,
	);

	const [minDuration, setMinDuration] = useState<number>(
		initialGranularity.minDuration,
	);
...
function TasksSearchSection({
	...
	return (
		<div className="px-3">
			...
			<TextInput
				leftSection={
					<Search className="size-3.5 text-foreground-subtle" aria-hidden />
				}
			...
			<div className="space-y-3">
				...
				<div className="flex items-center px-1">
					<Slider
						...
						size="sm"
						className="w-full"
						thumbLabel={t("panels.tasks.timeRange")}
...
function TasksMinDurationSection({
	...
	return (
		<div className="px-3 pt-4">
			<div className="mb-3 flex items-center justify-between">
				<div className="flex items-center gap-1.5">
					<Flame className="size-3.5 text-foreground-subtle" aria-hidden />
					<p className="text-caption font-bold uppercase tracking-wider text-foreground-lighter">
			...
			<div className="space-y-3">
				...
				<div className="flex items-center px-1">
					<Slider
						...
						size="sm"
						className="w-full"
						thumbLabel={t("panels.tasks.minDuration")}
	...
	return (
		<div className="px-3 pt-4">
			<div className="mb-3 flex items-center gap-1.5">
				<ShieldAlert className="size-3.5 text-foreground-subtle" aria-hidden />
				<p className="text-caption font-bold uppercase tracking-wider text-foreground-lighter">
