...
describe("dashboard-workspace core", () => {
	it("builds week and month timeline windows", () => {
		...
		const week = buildTimelineView(timeline, now, {
			scale: "week",
			customStart: null,
			customEnd: null,
		});
		const month = buildTimelineView(timeline, now, {
			scale: "month",
			customStart: null,
			customEnd: null,
		});

		expect(week.windowEnd.getTime() - week.windowStart.getTime()).toBe(
			7 * 24 * 60 * 60 * 1000,
		);
		expect(month.windowEnd.getTime() - month.windowStart.getTime()).toBe(
			30 * 24 * 60 * 60 * 1000,
		);
	...
	it("uses duration minutes to size blocks when end time is missing", () => {
		...
		const view = buildTimelineView(timeline, now, {
			scale: "day",
			customStart: null,
			customEnd: null,
		});
	...
	it("groups tiles by focus ready scheduled recent log from tile data", () => {
		...
		const scheduled = Tile.create(
			TileId.fromString("tile-scheduled"),
			"Scheduled task",
		);
		...
		const groups = groupForWorkspace(
			[focus, recent, scheduled, ready, log],
			focus.core.id,
			now,
		);

		expect(groups.find((group) => group.id === "focus")?.tiles[0].core.id).toBe(
			focus.core.id,
		);
		expect(groups.find((group) => group.id === "ready")?.tiles[0].core.id).toBe(
			ready.core.id,
		);
		expect(
			groups.find((group) => group.id === "recent")?.tiles[0].core.id,
		).toBe(recent.core.id);
		expect(
			groups.find((group) => group.id === "scheduled")?.tiles[0].core.id,
		).toBe(scheduled.core.id);
		expect(groups.find((group) => group.id === "log")?.tiles[0].core.id).toBe(
			log.core.id,
		);
	});
	...
	it("builds tile change events from timeline", () => {
		...
		const changes = buildChanges(timeline, titleById);
	...
	it("builds project and tag list sections from tile labels", () => {
		const now = new Date("2026-04-06T10:00:00.000Z");
		const withProject = Tile.create(
			TileId.fromString("tile-project"),
			"Project tile",
		);
		withProject.annotation.labels = ["project:core", "backend"];
		const withTagOnly = Tile.create(
			TileId.fromString("tile-tag"),
			"Tagged tile",
		);
		withTagOnly.annotation.labels = ["frontend"];
		const plain = Tile.create(TileId.fromString("tile-plain"), "Plain tile");

		const projectSections = buildListSections(
			[withProject, withTagOnly, plain],
			null,
			now,
			"project",
		);
		const tagSections = buildListSections(
			[withProject, withTagOnly, plain],
			null,
			now,
			"tag",
		...
		expect(projectSections.some((section) => section.label === "core")).toBe(
			true,
		);
		expect(
			projectSections.some((section) => section.label === "No Project"),
		).toBe(true);
		expect(tagSections.some((section) => section.label === "#backend")).toBe(
			true,
		);
		expect(tagSections.some((section) => section.label === "Untagged")).toBe(
			true,
		);
	});
	...
	it("keeps state sections visible even when empty", () => {
		const now = new Date("2026-04-06T10:00:00.000Z");
		const ready = Tile.create(
			TileId.fromString("tile-ready-only"),
			"Only ready",
		);
		const sections = buildListSections([ready], null, now, "state");
		...
		expect(
			sections.find((section) => section.id === "ready")?.tiles.length,
		).toBe(1);
		expect(
			sections.find((section) => section.id === "focus")?.tiles.length,
		).toBe(0);
	});

	it("cycles section limits 8 then doubles and resets when exceeding count", () => {
		expect(nextSectionLimit(undefined, 40)).toBe(16);
		expect(nextSectionLimit(16, 40)).toBe(32);
		expect(nextSectionLimit(32, 40)).toBe(8);
		expect(nextSectionLimit(8, 6)).toBe(8);
	});
