...
export class CommandHandler {
  handle(envelope: CommandEnvelope, initialState: AppState): EventEnvelope[] {
    const events = this.generate(envelope, initialState);
    // Apply the reducer to update state with generated events
    let state = initialState;
    for (const evt of events) {
      state = reduce(state, evt.event);
    ...
    switch (command.type) {
        ...
        const breakTileId =
          ...
          Array.from(state.tiles.values()).find(
            (t) =>
              ...
              t.core.startedAt !== null &&
              t.core.completedAt === null,
          )?.core.id ??
          null;
      ...
      case "extend_phase": {
        ...
        const expectedEndAt = new Date(
          envelope.issued_at.getTime() + command.delta_min * 60 * 1000,
        );
        events.push(
          this.wrap(envelope, `tile:${command.tile_id}`, {
            type: "segment_ended",
