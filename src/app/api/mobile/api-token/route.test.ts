...
vi.mock("@/lib/cognito/access-token-verification", () => ({ verifyCognitoAccessToken }));
...
beforeEach(() => {
  ...
  vi.stubEnv("TASTILE_WEB_BRIDGE_SECRET", "server-only-secret");
});
...
describe("POST /api/mobile/api-token", () => {
  it("rejects requests without a Cognito access token", async () => {
    ...
    const response = await POST(new Request("https://app.test/api/mobile/api-token", { method: "POST" }));
  ...
  it("rejects a token Cognito does not verify", async () => {
    verifyCognitoAccessToken.mockResolvedValueOnce(null);
    const { POST } = await import("./route");
    const fetchMock = vi.spyOn(globalThis, "fetch");
  ...
  it("mints for the verified sub using only the server-side bridge secret", async () => {
    verifyCognitoAccessToken.mockResolvedValueOnce("verified-sub");
