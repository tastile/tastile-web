...
type ErrorKey = "no_session" | "session_expired" | "invalid_email_or_password" | "email_not_verified";
...
export function LoginPanel(props: {
  ...
  const [error, setError] = useState<string | null>(() => {
    ...
    if (key) return t(`auth.login.errors.${key}`);
    if (props.initialError) return t("auth.login.errors.generic", { code: props.initialError });
    return null;
  });
  ...
  async function handleEmailSignIn(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (result.error) {
        ...
        setError(
          key ? t(`auth.login.errors.${key}`) : t("auth.login.errors.fallback"),
        );
        return;
      }
