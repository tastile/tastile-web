        ...
        <div className="layout-grid-2 mt-16 gap-8 items-stretch">
          {/* Free Plan */}
           <div className="flex flex-col rounded-xl bg-surface-elevated p-8">
             <h2 className="text-2xl font-[590] text-foreground">{t('marketing.pricing.freePlan')}</h2>
          ...
          {/* Pro Plan */}
           <div className="flex flex-col relative overflow-hidden rounded-xl bg-surface-elevated p-8">
            ...
            <div className="mt-3 flex gap-2">
              ...
              >{t('marketing.pricing.monthly')}</button>
              <button
               ...
               >{t('marketing.pricing.yearly')} <span className="text-success">{t('marketing.pricing.yearlySave')}</span></button>
            </div>
            <div className="mt-4 flex items-baseline">
               <span className="text-4xl font-[590] text-foreground">{interval === 'monthly' ? '$5' : '$50'}</span>
               <span className="ml-2 text-foreground-muted">{interval === 'monthly' ? t('marketing.pricing.perMonth') : t('marketing.pricing.perYear')}</span>
            </div>

            <ul className="mt-8 space-y-4">
              {dict.proFeatures.map((f, i) => (
                <li key={i} className="flex items-start">
                  <svg className="h-6 w-6 text-success mr-3 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                  </svg>
