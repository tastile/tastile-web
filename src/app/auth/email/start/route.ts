...
export async function POST(request: NextRequest) {
  ...
  const desktopQuery = redirectUri === 'tastile://auth/callback' && state
  ...
  try {
    ...
    const response = NextResponse.redirect(`${origin}/auth/email/verify?email=${encodeURIComponent(email)}${desktopQuery}`, 303)
    const options = {
      httpOnly: true,
    ...
    if (error instanceof CognitoPublicError && error.code === 'UserNotConfirmedException') {
      return NextResponse.redirect(`${origin}/auth/confirm?email=${encodeURIComponent(email)}&error=not_confirmed${desktopQuery}`, 303)
    }
    console.error('Email OTP start failed', error)
    return NextResponse.redirect(`${origin}/auth/email?email=${encodeURIComponent(email)}&error=otp_unavailable${desktopQuery}`, 303)
  }
}
