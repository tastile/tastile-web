...
export async function POST(request: NextRequest) {
  ...
  const redirectUri = safeOAuthRedirectUri(form.get('redirect_uri')?.toString() ?? null, env.callbackUrl)
  ...
  const desktopQuery = isDesktop
    ...
    : ''
  if (!email) return NextResponse.redirect(`${origin}/auth/email?error=missing_email${desktopQuery}`, 303)
  if (!code) return NextResponse.redirect(`${origin}/auth/email/verify?email=${encodeURIComponent(email)}&error=missing_code${desktopQuery}`, 303)
  ...
  if (!session || expectedEmail !== email) {
    return NextResponse.redirect(`${origin}/auth/email?email=${encodeURIComponent(email)}&error=auth_failed${desktopQuery}`, 303)
  }

  try {
    const tokens = await completeEmailOtpSignIn(env, email, code, session)
    const claims = parseIdTokenClaims(tokens.idToken)
    const response = NextResponse.redirect(isDesktop
      ? buildDesktopSuccessPage(origin, redirectUri, state, tokens)
      : `${origin}/dashboard`, 303)
    ...
    return NextResponse.redirect(`${origin}/auth/email/verify?email=${encodeURIComponent(email)}&error=invalid_code${desktopQuery}`, 303)
  }
}
...
function buildDesktopSuccessPage(
  ...
  redirectUri: string,
  state: string,
  tokens: { idToken: string; accessToken: string; refreshToken: string | null; expiresIn: number }
): string {
