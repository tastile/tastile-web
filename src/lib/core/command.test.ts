...
describe("fromDaemonCommandRequest", () => {
	...
	it("maps create_tile request", () => {
		...
		expect(
			fromDaemonCommandRequest({ type: "create_tile", tileId, tile }),
		).toEqual({
			type: "create_tile",
			tile_id: tileId,
			tile,
	...
	it("maps delete_tile request", () => {
		...
		expect(
			fromDaemonCommandRequest({ type: "delete_tile", tileId, deletedAt }),
		).toEqual({
			type: "delete_tile",
			tile_id: tileId,
	...
	it("maps extend_phase request", () => {
		const tileId = TileId.fromString("tile-1");
		expect(
			fromDaemonCommandRequest({ type: "extend_phase", tileId, deltaMin: 10 }),
		).toEqual({
			type: "extend_phase",
			tile_id: tileId,
			delta_min: 10,
