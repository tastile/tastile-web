...
export type TileListGroupingMode = 'state' | 'project' | 'tag'

export interface TileListSection {
  id: string
  label: string
  tiles: Tile[]
}
...
export function buildTimelineView(
  ...
  now: Date,
  input: TimelineWindowInput = { scale: 'day', customStart: null, customEnd: null }
): TimelineView {
  const window = resolveTimelineWindow(now, input)
  const pxPerMinuteBase = pixelsPerMinuteForScale(input.scale, window.windowStart, window.windowEnd)
  const pxPerMinute = resolveReadablePxPerMinute(timeline, window.windowStart, window.windowEnd, now, pxPerMinuteBase)
  const canvasHeightPx = Math.max(360, minutesBetween(window.windowStart, window.windowEnd) * pxPerMinute)
...
function resolveTimelineEnd(item: TimelineItemSnapshot, now: Date): Date {
...
export function buildTileChanges(
  timeline: TimelineItemSnapshot[],
...
export function groupTilesForWorkspace(
  tiles: Tile[],
  ...
  for (const tile of tiles) {
    const updatedAt = inferTileUpdatedAt(tile, latestByTile)
    if (updatedAt !== null) {
      latestByTile.set(tile.core.id, updatedAt)
    }
  }

  const groups: Record<TileWorkspaceGroup['id'], Tile[]> = {
    focus: [],
    ready: [],
    scheduled: [],
    recent: [],
  ...
  for (const tile of tiles) {
    ...
    if (lifecycle === 'done') {
      ...
      if (completedAt > 0 && now.getTime() - completedAt <= collapseDoneAfterDays * 24 * 60 * 60 * 1000) {
    ...
    const updatedAtMs = inferTileUpdatedAt(tile, latestByTile)
  ...
  const byRecentDesc = (a: Tile, b: Tile) => {
    const diff = (inferTileUpdatedAt(b, latestByTile) ?? 0) - (inferTileUpdatedAt(a, latestByTile) ?? 0)
  ...
  const orderedGroups: TileWorkspaceGroup[] = [
...
export function buildTileListSections(
  tiles: Tile[],
  activeTileId: TileId | null,
  now: Date,
  mode: TileListGroupingMode
): TileListSection[] {
  if (mode === 'state') {
    const grouped = groupTilesForWorkspace(tiles, activeTileId, now)
    const byId = new Map(grouped.map(group => [group.id, group]))
    const orderedIds: Array<TileWorkspaceGroup['id']> = ['focus', 'ready', 'scheduled', 'recent', 'log']
...
export function nextTileSectionLimit(currentLimit: number | undefined, itemCount: number): number {
...
function inferTileUpdatedAt(tile: Tile, latestByTile: Map<TileId, number>): number | null {
  const values: number[] = []
...
function buildMarkers(
  ...
  while (cursor <= windowEnd) {
    markers.push({
      label: scale === 'day' ? formatTime(cursor) : cursor.toLocaleDateString([], { month: '2-digit', day: '2-digit' }),
...
function groupByProject(tiles: Tile[]): TileListSection[] {
  const buckets = new Map<string, Tile[]>()
  for (const tile of tiles) {
    const projectLabel = tile.annotation.labels.find(label => label.startsWith('project:'))
    const project = projectLabel ? projectLabel.slice('project:'.length).trim() || 'No Project' : 'No Project'
    if (!buckets.has(project)) buckets.set(project, [])
    buckets.get(project)!.push(tile)
  }
  return Array.from(buckets.entries())
    .map(([label, groupedTiles]) => ({ id: `project:${label.toLowerCase()}`, label, tiles: sortTilesByRecency(groupedTiles) }))
    .sort((a, b) => b.tiles.length - a.tiles.length || a.label.localeCompare(b.label, 'en', { sensitivity: 'base' }))
}

function groupByTag(tiles: Tile[]): TileListSection[] {
  const buckets = new Map<string, Tile[]>()
  for (const tile of tiles) {
  ...
  return Array.from(buckets.entries())
    .map(([label, groupedTiles]) => ({ id: `tag:${label.toLowerCase()}`, label, tiles: sortTilesByRecency(groupedTiles) }))
    .sort((a, b) => b.tiles.length - a.tiles.length || a.label.localeCompare(b.label, 'en', { sensitivity: 'base' }))
}

function sortTilesByRecency(tiles: Tile[]): Tile[] {
  const copy = [...tiles]
  copy.sort((a, b) => inferTileSortTimestamp(b) - inferTileSortTimestamp(a))
  return copy
}

function fallbackStateLabel(id: TileWorkspaceGroup['id']): string {
...
function inferTileSortTimestamp(tile: Tile): number {
