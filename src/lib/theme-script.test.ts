...
describe('resolveInitialThemeMode', () => {
  it('prefers new theme-mode storage over legacy store', () => {
    const mode = resolveInitialThemeMode('dark-black', JSON.stringify({ state: { theme: 'light' } }), false)
    expect(mode).toBe('dark-black')
  })

  it('maps legacy persisted theme to current mode', () => {
    const mode = resolveInitialThemeMode(null, JSON.stringify({ state: { theme: 'gray' } }), false)
