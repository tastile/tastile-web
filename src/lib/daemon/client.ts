import type { DaemonCommandRequest } from "../core/command";
import type { ExecutionSnapshot, ExecutionSyncStatus } from "../domain/execution";
...
export class DaemonClient {
  ...
  async readTiles(params?: {
    viewMode?: string;
    lifecycle?: string;
    limit?: number;
    search?: string;
  }): Promise<DaemonTilesResponse> {
...
function toCurrentDaemonCommand(command: DaemonCommandRequest): {
  path: string;
...
function parseIntegrationSettings(raw: unknown): GoogleCalendarIntegrationSettings {
  ...
  return {
    ...
    accountEmail: asNullableString(
      read(gc, "account_email", "accountEmail"),
      "google_calendar.account_email",
    ),
    lastSyncedAt: asNullableString(
      read(gc, "last_synced_at", "lastSyncedAt"),
      "google_calendar.last_synced_at",
    ),
  };
}
...
function parseSyncStatus(raw: unknown): ExecutionSyncStatus {
  ...
  const lastAttemptRaw =
    "last_attempt_at" in row
      ? row.last_attempt_at
      : "lastAttemptAt" in row
        ? row.lastAttemptAt
        : null;
  const lastSuccessRaw =
    "last_success_at" in row
      ? row.last_success_at
      : "lastSuccessAt" in row
        ? row.lastSuccessAt
        : null;
  const lastErrorRaw =
    "last_error" in row ? row.last_error : "lastError" in row ? row.lastError : null;
  const lastResultInput =
    "last_result" in row ? row.last_result : "lastResult" in row ? row.lastResult : null;
...
function parseTilesResponse(raw: unknown): DaemonTilesResponse {
  ...
  return {
    tiles: asArray(read(row, "tiles"), "tiles").map((item, i) =>
      parseTileView(item, `tiles[${i}]`),
    ),
    nextActionableTileId: asNullableString(
      readOptional(row, "next_actionable_tile_id") ??
        readOptional(row, "nextActionableTileId") ??
        null,
      "next_actionable_tile_id",
    ),
    nextActionableStartAt: asNullableString(
      readOptional(row, "next_actionable_start_at") ??
        readOptional(row, "nextActionableStartAt") ??
        null,
...
function parseExecutionView(raw: unknown): DaemonExecutionViewResponse {
  ...
  return {
    tilesInProgress: asArray(
      read(row, "tiles_in_progress", "tilesInProgress"),
      "tiles_in_progress",
    ).map((item, i) => parseTileView(item, `tiles_in_progress[${i}]`)),
    mainTile: toNullableRecord(readOptional(row, "main_tile") ?? readOptional(row, "mainTile"))
      ? parseTileView(read(row, "main_tile", "mainTile"), "main_tile")
      : null,
    ...
    mainTileStartedAt: asNullableString(
      readOptional(row, "main_tile_started_at") ?? readOptional(row, "mainTileStartedAt") ?? null,
      "main_tile_started_at",
    ),
    mainTileEndsAt: asNullableString(
      readOptional(row, "main_tile_ends_at") ?? readOptional(row, "mainTileEndsAt") ?? null,
      "main_tile_ends_at",
    ),
    pendingPromptId: asNullableString(
      readOptional(row, "pending_prompt_id") ?? readOptional(row, "pendingPromptId") ?? null,
      "pending_prompt_id",
    ),
    tileCount: toFiniteCounter(
      readOptional(row, "tile_count") ?? readOptional(row, "tileCount") ?? 0,
    ),
    eventCount: toFiniteCounter(
      readOptional(row, "event_count") ?? readOptional(row, "eventCount") ?? 0,
    ),
  };
}
...
function parsePendingPrompt(raw: unknown): DaemonPendingPromptResponse {
  ...
  return {
    prompt: {
      ...
      tileId: asNullableString(
        readOptional(promptRow, "tile_id") ?? readOptional(promptRow, "tileId") ?? null,
        "tile_id",
      ),
      title: asString(read(promptRow, "title"), "title"),
      body: asString(read(promptRow, "body"), "body"),
      why: asString(read(promptRow, "why"), "why"),
      suggestedMinutes: asNullableNumber(
        readOptional(promptRow, "suggested_minutes") ??
          readOptional(promptRow, "suggestedMinutes") ??
          null,
        "suggested_minutes",
      ),
      reasons: asArray(readOptional(promptRow, "reasons") ?? [], "reasons").map((item, i) =>
        asString(item, `reasons[${i}]`),
      ),
      actions: asArray(readOptional(promptRow, "actions") ?? [], "actions").map((item, i) =>
        parsePromptAction(item, `actions[${i}]`),
      ),
      createdAt: asNullableString(
        readOptional(promptRow, "created_at") ?? readOptional(promptRow, "createdAt") ?? null,
        "created_at",
      ),
      expiresAt: asNullableString(
        readOptional(promptRow, "expires_at") ?? readOptional(promptRow, "expiresAt") ?? null,
        "expires_at",
      ),
  ...
  return {
    items: asArray(read(row, "items"), "items").map((item, i) => {
      ...
      return {
        ...
        tileId: asNullableString(
          readOptional(timeline, "tile_id") ?? readOptional(timeline, "tileId") ?? null,
          "tile_id",
        ),
        semanticRole: asNullableString(
          readOptional(timeline, "semantic_role") ?? readOptional(timeline, "semanticRole") ?? null,
          "semantic_role",
        ),
        title: asString(read(timeline, "title"), "title"),
        startedAt: asString(read(timeline, "started_at", "startedAt"), "started_at"),
        endedAt: asNullableString(
          readOptional(timeline, "ended_at") ?? readOptional(timeline, "endedAt") ?? null,
          "ended_at",
        ),
        durationMin: toFiniteCounter(
          readOptional(timeline, "duration_min") ?? readOptional(timeline, "durationMin") ?? 0,
        ),
        isActive: asBoolean(
          readOptional(timeline, "is_active") ?? readOptional(timeline, "isActive") ?? false,
          "is_active",
        ),
      };
    }),
...
function parseTileView(raw: unknown, field: string): DaemonTileView {
  ...
  const workedFallback = Math.max(
    0,
    Math.trunc(
      Number(readOptional(row, "worked_minutes") ?? readOptional(row, "workedMinutes") ?? 0),
    ),
  );
  return {
    ...
    nextAction: asNullableString(
      readOptional(row, "next_action") ?? readOptional(row, "nextAction") ?? null,
      `${field}.next_action`,
    ),
    doneDefinition: asNullableString(
      readOptional(row, "done_definition") ?? readOptional(row, "doneDefinition") ?? null,
      `${field}.done_definition`,
    ),
    workedMinutes: toFiniteCounter(
      readOptional(row, "worked_minutes") ?? readOptional(row, "workedMinutes") ?? 0,
    ),
    breakMinutes: toFiniteCounter(
      readOptional(row, "break_minutes") ?? readOptional(row, "breakMinutes") ?? 0,
    ),
    semanticRole: asString(
      readOptional(row, "semantic_role") ?? readOptional(row, "semanticRole") ?? "work",
      `${field}.semantic_role`,
    ),
    labels: asArray(readOptional(row, "labels") ?? [], `${field}.labels`).map((item, i) =>
      asString(item, `${field}.labels[${i}]`),
    ),
    objectiveMode: asNullableString(
      readOptional(row, "objective_mode") ?? readOptional(row, "objectiveMode") ?? null,
      `${field}.objective_mode`,
    ),
    targetWorkMin: targetWork ?? (workedFallback > 0 ? workedFallback : null),
    targetRestMin: asNullableNumber(
      readOptional(row, "target_rest_min") ?? readOptional(row, "targetRestMin") ?? null,
      `${field}.target_rest_min`,
    ),
    doneRule: asNullableString(
      readOptional(row, "done_rule") ?? readOptional(row, "doneRule") ?? null,
      `${field}.done_rule`,
    ),
    resumeNote: asNullableString(
      readOptional(row, "resume_note") ?? readOptional(row, "resumeNote") ?? null,
      `${field}.resume_note`,
    ),
    projectedNextStartAt: asNullableString(
      readOptional(row, "projected_next_start_at") ??
        readOptional(row, "projectedNextStartAt") ??
        null,
      `${field}.projected_next_start_at`,
    ),
    temporal: temporalRow
      ? {
          tz: asNullableString(readOptional(temporalRow, "tz") ?? null, "temporal.tz"),
          releaseAt: asNullableString(
            readOptional(temporalRow, "release_at") ??
              readOptional(temporalRow, "releaseAt") ??
              null,
            "temporal.release_at",
          ),
          dueAt: asNullableString(
            readOptional(temporalRow, "due_at") ?? readOptional(temporalRow, "dueAt") ?? null,
            "temporal.due_at",
          ),
          fixedStart: asNullableString(
            readOptional(temporalRow, "fixed_start") ??
              readOptional(temporalRow, "fixedStart") ??
              null,
            "temporal.fixed_start",
          ),
          fixedEnd: asNullableString(
            readOptional(temporalRow, "fixed_end") ?? readOptional(temporalRow, "fixedEnd") ?? null,
            "temporal.fixed_end",
          ),
          activeStart: asNullableString(
            readOptional(temporalRow, "active_start") ??
              readOptional(temporalRow, "activeStart") ??
              null,
            "temporal.active_start",
          ),
          activeEnd: asNullableString(
            readOptional(temporalRow, "active_end") ??
              readOptional(temporalRow, "activeEnd") ??
              null,
            "temporal.active_end",
          ),
        }
