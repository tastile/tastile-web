...
describe('DaemonClient', () => {
  ...
  it('reads desktop parity read models', async () => {
    ...
    const client = new DaemonClient({ baseUrl: 'https://daemon.example', fetchImpl })

    const tiles = await client.readTiles({ viewMode: 'list', lifecycle: 'ready', limit: 8, search: 'write' })
    ...
    expect(pendingPrompt.prompt?.actions[0]).toEqual({ id: 'start_tile', label: 'Start' })
