...
export function parseExecutionSnapshot(raw: unknown): ExecutionSnapshot {
  ...
  const inProgressTiles = asArray(read(root, 'in_progress_tiles'), 'in_progress_tiles').map(parseInProgressTile)
...
function asPromptQueueStatus(value: unknown): PromptQueueStatus {
  ...
  if (status === 'pending' || status === 'acknowledged' || status === 'completed' || status === 'dismissed') {
    return status
  }
