...
import { type GroupedMonthEvents, getMonthPositionedEvents } from "./get-month-positioned-events";
  ...
  for (const event of events) {
    if (
      ...
      if (!ids.has(event.id)) {
        ...
        throw new Error(
          `[@/lib/vendored/mantine-schedule] MonthView: Duplicated event ids found: ${event.id}`,
        );
      }
    }
  }
