...
import type { AnyDateValue, DateStringValue, ScheduleEventData } from "../../types";
  ...
  for (const event of events) {
    ...
    if (dayjs(event.start).isSame(dayjs(date), "month")) {
      ...
      if (!ids.has(event.id)) {
        ...
        throw new Error(
          `[@/lib/vendored/mantine-schedule] MobileMonthView: Duplicated event ids found: ${event.id}`,
        );
      }
    }
  }
