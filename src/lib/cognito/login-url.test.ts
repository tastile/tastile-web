...
describe('cognito login url', () => {
  ...
  it('keeps callback next paths local', () => {
    expect(safeNextPath('/dashboard/tiles?tab=list')).toBe('/dashboard/tiles?tab=list')
    expect(safeNextPath('https://evil.example')).toBe('/dashboard')
    expect(safeNextPath('//evil.example')).toBe('/dashboard')
  })

  it('allows the desktop callback URI but rejects arbitrary OAuth redirects', () => {
    expect(safeOAuthRedirectUri('tastile://auth/callback', env.callbackUrl)).toBe('tastile://auth/callback')
    expect(safeOAuthRedirectUri('https://evil.example/callback', env.callbackUrl)).toBe(env.callbackUrl)
  })
  ...
  it('passes external redirect_uri into Cognito URLs for desktop PKCE', () => {
    ...
    expect(url.searchParams.get('redirect_uri')).toBe('tastile://auth/callback')
  ...
  it('validates PKCE URL values before accepting external client state', () => {
    expect(safePkceValue('abcDEF123._~-abcDEF123')).toBe('abcDEF123._~-abcDEF123')
