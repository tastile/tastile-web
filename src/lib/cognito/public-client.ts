...
export async function signUpPasswordlessEmail(env: CognitoEnv, email: string): Promise<PasswordlessSignUpResult> {
...
export async function startEmailOtpSignIn(env: CognitoEnv, email: string): Promise<EmailOtpStartResult> {
  ...
  throw new CognitoPublicError('Email OTP sign-in is not available for this user.', 'EMAIL_OTP_UNAVAILABLE')
}
...
async function cognitoRequest(env: CognitoEnv, target: string, body: CognitoJson): Promise<CognitoJson> {
