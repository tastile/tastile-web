...
describe('cognito cookies', () => {
  ...
  it('omits refresh cookie when refreshToken is null', async () => {
    await setAuthCookies({ idToken: 'id', refreshToken: null, sub: 'sub-1', expiresIn: 3600 })
  ...
  it('reads id_token', async () => {
    await setAuthCookies({ idToken: 'my-id', refreshToken: null, sub: 'sub', expiresIn: 60 })
    expect(await getIdTokenFromCookies()).toBe('my-id')
  })

  it('reads refresh_token and sub', async () => {
    await setAuthCookies({ idToken: 'i', refreshToken: 'my-ref', sub: 'my-sub', expiresIn: 60 })
