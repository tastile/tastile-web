...
import type { Result } from "@/lib/api/endpoints";
import type { V1ActiveTileSnapshot } from "@/lib/api/v1/active-tile";

const { fetchV1ActiveTileMock } = vi.hoisted(() => ({
  fetchV1ActiveTileMock: vi.fn(),
}));

vi.mock("@/lib/api/v1/active-tile", async () => {
  const actual = await vi.importActual<typeof import("@/lib/api/v1/active-tile")>(
    "@/lib/api/v1/active-tile",
  );
  return {
    ...actual,
