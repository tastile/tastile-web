...
describe("buildDashboardProjection", () => {
	it("selects next main + 5 from ready tiles", () => {
		...
		for (const title of [
			...
			const tile = Tile.create(
				TileId.fromString(`tile-${title.toLowerCase()}`),
				title,
			);
			state.tiles.set(tile.core.id, tile);
		}

		const view = buildDashboardProjection(
			state,
			new Date("2026-03-26T09:00:00.000Z"),
		);
		expect(view.next.main?.core.title).toBe("Alpha");
		expect(view.next.quick.map((tile) => tile.core.title)).toEqual([
			"Bravo",
			"Charlie",
			"Delta",
			"Echo",
	...
	it("builds absolute timeline blocks with visual heights", () => {
		...
		expect(view.timeline.blocks[0].heightPx).toBeGreaterThan(
			view.timeline.blocks[1].heightPx,
		);
