...
function statusLabel(
  ...
  return t(
    `account.subscription.status${status.charAt(0).toUpperCase()}${status.slice(1)}`,
  );
}
...
export function SubscriptionSection() {
  ...
  const [intervalChoice, setIntervalChoice] = useState<"monthly" | "yearly">(
    "monthly",
  );
  ...
  const error =
    actionError ??
    (subscriptionQuery.isError ? t("account.subscription.error") : null);
  ...
  const proInterval =
    state && state.status !== "free" ? state.interval : intervalChoice;
          ...
          <p className="text-xs text-foreground-muted mb-4">
            ...
            {state.currentPeriodEnd > 0 && (
              <>
                ...
                {formatDate(state.currentPeriodEnd, locale)}
              </>
            )}
          </p>
        )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <section className="border border-border bg-surface-0 rounded-md p-6">
          ...
          <div className="flex gap-2 mb-3">
            <SegmentedControl
              onChange={(value) =>
                setIntervalChoice(value as "monthly" | "yearly")
              }
              value={proInterval}
              data={[
