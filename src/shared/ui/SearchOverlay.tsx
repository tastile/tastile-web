...
const DASHBOARD_ROUTES: Array<{
  path: string;
  labelKey: SearchOverlayRouteKey;
}> = [
...
function SearchOverlayInner({ onClose }: { onClose: () => void }) {
  ...
  const filteredRoutes = useMemo(
    () =>
      routes.filter((r) =>
        deferredQuery
          ? r.label.toLowerCase().includes(deferredQuery.toLowerCase())
          : false,
      ),
  ...
  return (
    ...
    <dialog
      ...
      className="mx-auto mt-24 max-w-[600px] w-[92vw] rounded-xl border border-border bg-surface-elevated text-left [&::backdrop]:bg-foreground/5 [&::backdrop]:backdrop-blur-sm p-0"
    >
