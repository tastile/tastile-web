...
import { projectsQueryOptions } from "@/shared/query/projects-query-options";
...
export async function createWorkspace(input: CreateWorkspaceInput): Promise<Workspace> {
  return createWorkspaceRequest(input);
}
...
export function orderWorkspaceTree(workspaces: Workspace[]): WorkspaceTreeEntry[] {
  ...
  for (const children of byParent.values())
    children.sort((a, b) => a.display_name.localeCompare(b.display_name, "ja"));
