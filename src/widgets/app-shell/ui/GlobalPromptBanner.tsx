...
export function GlobalPromptBanner({
	prompt,
	onAction,
	onDismiss,
}: GlobalPromptBannerProps) {
	const { t } = useTranslation();
	const [deferMenuPromptId, setDeferMenuPromptId] = useState<string | null>(
		null,
	);
	...
	const visibleActions = prompt.actions.filter(
		(action) => action !== "dismiss",
	);
	const canDismiss =
		prompt.actions.includes("dismiss") && typeof onDismiss === "function";
	...
	return (
		<div
			role="status"
			aria-live="polite"
			className="fixed top-4 right-4 z-[70] w-[min(92vw,420px)]"
		>
			<div
				className={`rounded-2xl p-4 backdrop-blur ${SEVERITY_STYLE[prompt.severity]}`}
			>
						...
						<div className="grid grid-cols-2 gap-2">
							{deferOptions.map((option) => (
								<Button
									...
									type="button"
									onClick={() =>
										onAction?.("defer_tile", { deferMinutes: option.minutes })
									}
