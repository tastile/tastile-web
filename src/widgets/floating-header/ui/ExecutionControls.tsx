...
import {
	snapshotFromActiveTile,
	useV1Execution,
} from "@/shared/hooks/use-v1-execution";
		...
		<div className="flex items-center gap-1.5">
						...
						void run("start")
							.then(() => {
								window.dispatchEvent(
									new CustomEvent("tastile:execution-changed"),
								);
							})
							.catch((err: unknown) => {
			...
			{state.error && (
				<span
					role="alert"
					aria-live="polite"
					className="hidden text-caption text-status-danger lg:inline"
					title={state.error}
				>
					{state.error}
				</span>
