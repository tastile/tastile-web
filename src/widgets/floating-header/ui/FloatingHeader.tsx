...
export function FloatingHeader({
	...
	const ends = snapshot?.main_tile_ends_at
		? new Date(snapshot.main_tile_ends_at)
		: null;
	...
	const remainingSec = ends
		? Math.max(0, Math.round((ends.getTime() - nowMs) / 1000))
		: 0;
	...
	const status = isWorking
		? t("shell.floatingHeader.executing")
		: t("shell.floatingHeader.idle");

	return (
		<>
			<header className="fixed inset-x-0 top-0 z-40 flex h-12 items-center bg-surface-0 pt-[env(safe-area-inset-top)]">
				{/* Left: logo */}
					...
					<div className="flex items-center gap-2 font-mono text-caption text-foreground-muted">
						...
						{main ? (
							<>
								...
								<span className="truncate max-w-[8rem] text-foreground sm:max-w-[200px]">
									{main.title}
								</span>
								{ends ? (
									<>
				...
				<div className="flex items-center gap-1 pr-2 sm:pr-3">
					...
					<FloatingMenu>
						<FloatingMenuTrigger asChild>
							<Avatar aria-label={t("shell.floatingHeader.userMenu")}>
								{userName.charAt(0)}
							</Avatar>
						</FloatingMenuTrigger>
