...
import { mapRange, easeOut, lerp } from "@/lib/animate";
import { Calendar, Clock, ListTodo } from "lucide-react";

function op(p: number, s: number, e: number) { return easeOut(mapRange(p, s, e)); }
function ty(p: number, s: number, e: number, px = 40) { return lerp(px, 0, easeOut(mapRange(p, s, e))); }
...
type Dict = {
  ...
  bridgeDesc: string;
  howTitle: string;
  howPrinciples: readonly { symbol: string; text: { ja: string; en: string } }[];
...
const apps = [
  { icon: <Calendar className="w-8 h-8" />, name: { ja: "カレンダー", en: "Calendar" }, strength: { ja: "「いつ」を可視化", en: "Visualizes 'when'" }, gap: { ja: "中身は知らない", en: "Doesn't know content" } },
  { icon: <Clock className="w-8 h-8" />, name: { ja: "時計", en: "Clock" }, strength: { ja: "「今」を動かす", en: "Moves 'now'" }, gap: { ja: "何をやるか知らない", en: "Doesn't know what" } },
  { icon: <ListTodo className="w-8 h-8" />, name: { ja: "タスク管理", en: "Tasks" }, strength: { ja: "残りを見る", en: "Shows what's left" }, gap: { ja: "いつやるか委ねられる", en: "When is up to you" } },
];

export function ScrollPage({ t, stepIcons, lang }: {
  t: Dict; stepIcons: ReactNode[]; lang: string;
}) {
        ...
        <div className="sticky top-0 h-screen flex items-center z-10">
          <div className="layout-shell w-full">
            <div className="max-w-[600px]">
              ...
              <p className="mt-8 max-w-[480px] text-lg leading-relaxed text-foreground-muted">{t.heroDesc}</p>
              <div className="mt-12 flex flex-wrap gap-4">
                <a href="/login" className="inline-flex items-center gap-2 rounded-full bg-foreground px-8 py-3.5 text-sm font-medium text-background transition-colors duration-150 hover:bg-interactive-hover">
                  {t.heroCta1}
                </a>
                <a href="/download" className="inline-flex items-center gap-2 rounded-full bg-surface-0 px-8 py-3.5 text-sm font-medium text-foreground-muted transition-colors duration-150 hover:bg-surface-2 hover:text-foreground">
                  {t.heroCta2}
                </a>
              </div>
            </div>
        ...
        >
          <div className="rounded-2xl bg-surface-elevated p-6 shadow-[0_8px_40px_rgba(0,0,0,0.06)]">
            <div className="flex items-center gap-3 mb-5">
              <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-surface-2 text-foreground-muted">{stepIcons[0]}</span>
              <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-surface-2 text-foreground-muted">{stepIcons[1]}</span>
              <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-surface-2 text-foreground-muted">{stepIcons[2]}</span>
            </div>
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  <p className="text-sm font-medium text-foreground">
                    {lang === "ja" ? "競合調査レポートをまとめる" : "Write competitive analysis report"}
                  </p>
  ...
  return (
    <div ref={ref} className="relative py-32" style={{ minHeight: "300vh" }}>
      <div className="sticky top-0 h-screen flex items-center">
        <div className="layout-shell w-full">
          {/* Title */}
          <h2 className="font-[family-name:var(--font-jp-heading)] mb-16 text-3xl font-semibold tracking-tight text-foreground lg:text-4xl"
          ...
          <div className="flex justify-center items-center gap-5 max-w-4xl mx-auto mt-16"
            style={{
          ...
          <p className="text-center mt-8 text-base text-foreground-muted max-w-lg mx-auto"
    ...
    <div ref={ref} className="relative py-32" style={{ minHeight: "200vh" }}>
      <div className="sticky top-0 h-screen flex items-center justify-center">
        <div className="text-center max-w-2xl px-8">
          <h2 className="font-[family-name:var(--font-jp-heading)] text-3xl font-semibold tracking-tight text-foreground lg:text-5xl"
            style={{ opacity: op(p, 0.1, 0.35), transform: `translateY(${ty(p, 0.1, 0.35, 30)}px)` }}>
            {t.bridgeTitle}
          </h2>
          <p className="mt-8 text-lg text-foreground-muted leading-relaxed"
            style={{ opacity: op(p, 0.25, 0.5), transform: `translateY(${ty(p, 0.25, 0.5, 20)}px)` }}>
            {t.bridgeDesc}
          </p>
        </div>
      </div>
...
function HowReveal({ t, lang }: { t: Dict; lang: "ja" | "en" }) {
  ...
  return (
    <div ref={ref} className="relative py-32" style={{ minHeight: "350vh" }}>
      <div className="sticky top-0 h-screen flex items-center">
        <div className="layout-shell w-full">
          <h2 className="font-[family-name:var(--font-jp-heading)] mb-12 text-center text-3xl font-semibold tracking-tight text-foreground lg:text-4xl"
            style={{ opacity: op(p, 0, 0.08) }}>
            {t.howTitle}
          </h2>
          ...
          <div className="flex justify-center items-center gap-3 mb-16 flex-wrap"
            style={{ opacity: op(p, 0.04, 0.16), transform: `translateX(${lerp(-40, 0, easeOut(mapRange(p, 0.04, 0.16)))}px)` }}>
            {t.howPipeline[lang].split(" → ").map((step, i, arr) => (
              <span key={i} className="flex items-center gap-3">
                <span className="rounded-xl bg-surface-elevated px-5 py-2.5 text-sm font-medium text-foreground shadow-[0_2px_8px_rgba(0,0,0,0.04)]">{step}</span>
            ...
            {t.howPrinciples.map((item, i) => {
              ...
              return (
                <div key={i} className="flex items-start gap-5"
                  ...
                  <span className="text-3xl text-foreground-subtle flex-none mt-0.5">{item.symbol}</span>
                  <p className="text-base text-foreground-muted leading-relaxed">{item.text[lang]}</p>
                </div>
              );
            })}
          </div>
                ...
                <div key={i} className="flex items-center"
                  ...
                  {isArrow ? (
                    ...
                    <div className="text-center">
                      ...
                      {step.body && <p className="mt-1 text-xs text-foreground-muted">{step.body}</p>}
                    </div>
                  )}
                </div>
              );
    ...
    <div ref={ref} className="relative py-32" style={{ minHeight: "250vh" }}>
      <div className="sticky top-0 h-screen flex items-center justify-center">
        <div className="text-center max-w-xl px-8">
          <h2 className="font-[family-name:var(--font-jp-heading)] mb-12 text-3xl font-semibold tracking-tight text-foreground lg:text-4xl"
            style={{ opacity: op(p, 0.03, 0.1) }}>
            {t.lifeTitle}
          </h2>
          <div className="space-y-8">
            {t.lifeLines.map((line, i) => {
              ...
              const e = s + 0.15;
              return (
                <p key={i} className="text-center text-lg text-foreground-muted"
                  style={{ opacity: op(p, s, e), transform: `translateY(${ty(p, s, e, 16)}px)` }}>
                  {line}
                </p>
              );
            })}
    ...
    <div ref={ref} className="relative py-32" style={{ minHeight: "200vh" }}>
      <div className="sticky top-0 h-screen flex items-center justify-center">
        <div className="text-center max-w-lg px-8">
          <h2 className="font-[family-name:var(--font-jp-heading)] mb-4 text-3xl font-semibold tracking-tight text-foreground lg:text-4xl"
            style={{ opacity: op(p, 0.05, 0.25), transform: `translateY(${ty(p, 0.05, 0.25, 20)}px)` }}>
            {t.pricingTitle}
          </h2>
          <p className="text-lg text-foreground-muted"
            style={{ opacity: op(p, 0.12, 0.35), transform: `translateY(${ty(p, 0.12, 0.35, 16)}px)` }}>
            {t.pricingBody}
          </p>
          <div className="mt-10"
            style={{ opacity: op(p, 0.25, 0.45), transform: `translateY(${ty(p, 0.25, 0.45, 12)}px)` }}>
            <a href="/pricing" className="inline-flex items-center gap-2 rounded-full bg-foreground px-10 py-4 text-sm font-medium text-background transition-colors duration-150 hover:bg-interactive-hover">
    ...
    <div ref={ref} className="relative py-32" style={{ minHeight: "300vh" }}>
      <div className="sticky top-0 h-screen flex items-center">
        <div className="layout-shell max-w-2xl w-full">
          <h2 className="font-[family-name:var(--font-jp-heading)] mb-10 text-center text-3xl font-semibold tracking-tight text-foreground lg:text-4xl"
          ...
          <div className="space-y-3">
            {t.qa.map((item, i) => {
              ...
              const e = s + 0.12;
              return (
                <div key={item.q} className="rounded-2xl bg-surface-elevated px-7 py-6"
                  style={{
                    opacity: op(p, s, e),
    ...
    <div ref={ref} className="relative py-32" style={{ minHeight: "200vh" }}>
      <div className="sticky top-0 h-screen flex items-center justify-center">
        <div className="text-center max-w-3xl px-8">
          <h2 className="font-[family-name:var(--font-jp-heading)] text-4xl font-semibold leading-tight tracking-tight text-foreground lg:text-6xl"
            style={{ opacity: op(p, 0.08, 0.35) }}>
            {t.ctaTitle.map((l, i) => (
              <span key={i} className="block"
                style={{ opacity: op(p, 0.1 + i * 0.08, 0.35 + i * 0.08), transform: `translateY(${ty(p, 0.1 + i * 0.08, 0.35 + i * 0.08, 20)}px)` }}>
                {l}
              </span>
            ))}
          </h2>
          <p className="mt-8 text-lg text-foreground-muted"
            style={{ opacity: op(p, 0.3, 0.5), transform: `translateY(${ty(p, 0.3, 0.5, 16)}px)` }}>
            {t.ctaBody}
          </p>
          <div className="mt-12 flex justify-center flex-wrap gap-4"
            style={{ opacity: op(p, 0.38, 0.58), transform: `translateY(${ty(p, 0.38, 0.58, 12)}px)` }}>
            <a href="/login" className="inline-flex items-center gap-2 rounded-full bg-foreground px-10 py-4 text-sm font-medium text-background transition-colors duration-150 hover:bg-interactive-hover">
              {t.ctaCta1}
            </a>
            <a href="/download" className="inline-flex items-center gap-2 rounded-full bg-surface-0 px-10 py-4 text-sm font-medium text-foreground-muted transition-colors duration-150 hover:bg-surface-2 hover:text-foreground">
              {t.ctaCta2}
            </a>
          </div>
        </div>
