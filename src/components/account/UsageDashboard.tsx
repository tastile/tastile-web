    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {placeholderCharts.map((chart, index) => (
          <div
            key={index}
            className="p-6 bg-surface-secondary rounded-lg border border-border min-h-[200px] flex flex-col"
          >
            <div className="flex-1">
              <h3 className="text-lg font-semibold text-foreground mb-2">{chart.title}</h3>
              <p className="text-sm text-foreground-tertiary mb-4">{chart.description}</p>

              {/* Placeholder Chart Area */}
              <div className="h-32 bg-surface-tertiary rounded flex items-center justify-center">
                <div className="text-center">
                  <div className="text-3xl text-foreground-tertiary mb-2">📊</div>
                  <p className="text-xs text-foreground-tertiary">Chart placeholder</p>
                </div>
              </div>
            </div>
          </div>
