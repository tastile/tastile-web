...
export function ActiveExecutionBar({
  ...
  if (!activeTileTitle || !phaseStartedAt || !phaseEndsAt) {
    if (!activeTileTitle || (!phaseStartedAt && !phaseEndsAt)) {
      const fallbackLabel = resolveCountdownLabel(phaseEndsAt, nextActionableStartAt, new Date(nowMs))
      if (mode === 'header-left') {
        return (
          <div className="flex min-w-0 items-center gap-2">
            <span className="text-2xl font-mono font-semibold tabular-nums text-foreground shrink-0">{fallbackLabel}</span>
      ...
      return (
        <div className="flex w-full min-w-0 max-w-2xl items-center justify-end py-4">
          <span className="text-3xl font-mono font-semibold tabular-nums text-foreground">{fallbackLabel}</span>
        </div>
      )
    }
    ...
    if (mode === 'header-left') {
      return (
        <div className="flex min-w-0 items-center gap-2">
          <span className="text-2xl font-mono font-semibold tabular-nums text-foreground shrink-0">{runningLabel}</span>
    ...
    return (
      <div className="flex w-full min-w-0 max-w-2xl items-center gap-4 py-4">
        ...
        <span className="min-w-0 flex-1 truncate text-base font-semibold text-foreground">実行中: {activeTileTitle}</span>
        <span className="text-3xl font-mono font-semibold tabular-nums text-foreground shrink-0">{runningLabel}</span>
      </div>
    )
  }
  ...
  if (mode === 'header-left') {
      return (
        <div className="flex min-w-0 items-center gap-2">
          <span className="text-[10px] font-semibold uppercase tracking-wider text-foreground-muted shrink-0">残り</span>
          <span className="text-2xl font-mono font-semibold tabular-nums text-foreground shrink-0">{metrics.countdownLabel}</span>
  ...
  return (
      ...
      <span className="text-3xl font-mono font-semibold tabular-nums text-foreground shrink-0">{metrics.countdownLabel}</span>
    </div>
  )
}
