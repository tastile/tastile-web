...
describe("GlobalPromptBanner", () => {
	it("renders prompt details and action buttons", () => {
		...
		expect(
			screen.getByRole("button", { name: "tiles.actions.start" }),
		).toBeTruthy();
		expect(
	...
	it("calls handlers for prompt actions", () => {
		...
		fireEvent.click(
			screen.getByRole("button", { name: "tiles.actions.complete" }),
		);
		fireEvent.click(
			screen.getByRole("button", { name: "prompt.actions.extend" }),
		);
	...
	it("AppShell maps defer prompt action to defer_tile with computed next_start_at", async () => {
		...
		const deferDeltaMin = Math.floor(
			(firstCommand.next_start_at.getTime() - clickedAt) / 60000,
		);
	...
	it("AppShell sends respond_startup_recovery confirm_continue then clears prompt", async () => {
		...
		fireEvent.click(
			screen.getByRole("button", { name: "prompt.actions.confirmContinue" }),
		);
		await waitFor(() => expect(executeMock).toHaveBeenCalledTimes(2));
		expect(executeMock).toHaveBeenNthCalledWith(
			1,
