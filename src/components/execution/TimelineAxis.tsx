...
export function TimelineAxis({
  ...
  const effective = useMemo(() => {
    ...
    return source.filter(block => block.endAt.getTime() >= start && block.startAt.getTime() <= end)
  }, [blocks, scope])
  ...
  if (visible.length > 0) {
    ...
    const visibleMarkers = markers.filter(marker => marker.topPx >= viewportMin && marker.topPx <= viewportMax)
          ...
          {visible.map(block => {
            ...
            return (
              ...
              >
                ...
                <p className="text-[10px] uppercase tracking-wider text-foreground-muted">{block.type}</p>
              </div>
            )
          })}
        </div>
    ...
    <ol className="space-y-3">
      {legacyItems.map(item => (
        <li key={item.id} className="flex items-start gap-3">
          <div data-testid="timeline-time" className="w-24 shrink-0 text-xs font-mono text-foreground-muted">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[11px] uppercase tracking-wider text-foreground-muted">{item.type}</p>
          </div>
        </li>
      ))}
    </ol>
