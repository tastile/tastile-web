...
export function EventListView({ events, loading, error }: EventListViewProps) {
  ...
  if (loading) {
    return <div className="py-12 text-center text-xs text-foreground-subtle">Loading…</div>;
  }
  if (error) {
    return (
  ...
  if (groups.length === 0) {
    return (
      <div className="flex flex-col items-center gap-2 py-16 text-center" data-testid="event-list">
        <Calendar className="h-8 w-8 text-foreground-subtle" />
        <p className="text-sm text-foreground-subtle">
          {query ? "No events match your search." : "No events yet. Click + to create one."}
        </p>
      </div>
    );
  }
