...
export const landingDict: Record<Lang, Dict> = {
	ja: {
		hero: {
			badge: "実行支援ツール",
			title: ["考えるのは、", "もうやめていい。"],
			sub: "やることを入れると、Tastileが時間帯まで自動で組みます。",
			context: "カレンダー・時計・タスク管理をひとつにまとめた、実行のためのエンジン。頭の中の「次になにをすべきか」を、Tastileが代わりに決めます。",
			pierceText: "実行",
			stats: [
				{ value: "4.2h", label: "集中時間", sub: "/ 1日あたり" },
				{ value: "0", label: "判断回数", sub: "あなたがした" },
				{ value: "94%", label: "完了率", sub: "走り切ったタイル" },
			],
			ctaPrimary: "始める",
			ctaSecondary: "ダウンロード",
		...
		bento: {
			eyebrow: "6つの軸",
			title: "タイルは、6つの軸で定義される。",
			intro: "ひとつのタスクは、6つの軸の組み合わせで形になる。",
			lead: "あなたは軸を定義する。エンジンが残りを担う。各軸は独立ではなく、互いに参照し合って、タイルの「形」を決めていく。",
			rows: [
				{
					numeral: "01",
					name: "期限",
					lede: "いつまでに終えるか",
					body: "タイル自身が期限を覚えている。エンジンは進捗と空き時間を逆算し、最も効く場所に置く。遅れたら即座に検知して、次の空きに自動で組み直す。期限は「厳守しなければならない数字」ではなく、タイルが自ら持つ「到着希望時刻」として動く。",
				...
				},
				{
					...
					name: "場所",
					lede: "どこでやるか",
					body: "どこでやるかも軸のひとつ。場所と時間帯の相性を見て、最も集中できる枠を選ぶ。屋外と屋内、集中型と移動型を区別する。場所は単なるタグではなく、タスクの「性格」を決める要素として扱われる。",
				...
				},
				{
					...
					name: "状態",
					lede: "開始で何が変わるか",
					body: "開始すると、何が変わるか。状態に合わせて周囲の環境を自動で整える。集中モードに入る、通知を抑える、関連する資料を開く。タイルの開始は「あなたが何かをする」のではなく「世界がそのタスク向けに切り替わる」体験になる。",
				...
				},
				{
					...
					name: "変形",
					lede: "途中で形を変える",
					body: "途中で形を変えてもよい。状況に応じて削れる部分を削り、不足を補う。60分の予定が45分で済みそうなら、残りを別タスクに振る。変形は「失敗」ではなく「最適化」として扱われ、履歴にすべて残る。",
				...
				},
				{
					...
					name: "完了",
					lede: "何を以て終わりとするか",
					body: "何が「終わり」かを、タイル自身が定義する。曖昧なまま終わりにしない。チェックリスト、出力ファイル、外部への通知。複数の条件を持てる。「完了」は人間の主観ではなく、タイルの仕様として記述される。",
				...
				},
				{
					...
					name: "超過",
					lede: "はみ出した分を自動で次へ",
					body: "間に合わなかった時の振る舞いも、タイルが自分で決める。45分の予定が55分かかっても、積み残さず次の空き枠へ。タスクが消えることはなく、常に状態を持つ。超過は「失敗」ではなく、エンジンの正常な動作。",
			...
			overflow: {
				name: "超過処理",
				lede: "はみ出した分を自動で次へ送る",
				body: "間に合わなかった時の振る舞いも、タイルが自分で決める。45分の予定が55分かかっても、積み残さず次の空き枠へ。タスクが消えることはなく、常に状態を持つ。",
				example: "45分の予定が55分かかる → 残り5分は12:00の空き枠に自動で組み込み。履歴にも残る。",
			},
		},
		lifecycle: {
