...
export function CtaSection({ t, lang }: { t: Dict["finalCta"]; lang: Lang }) {
	...
	return (
		<section className="layout-shell relative overflow-hidden pt-24 pb-32 lg:pt-32 lg:pb-40">
			...
			>
				<p className={`mkt-pierce-stroke mkt-pierce-accent whitespace-nowrap text-center ${display}`}>
					{t.pierceText}
				</p>
			</div>
			...
			<div className="relative z-10">
				<div className="max-w-3xl">
					...
					<h2
						className={`mt-4 mkt-display-1 text-foreground ${display}`}
					>
						<span className="block">{t.title[0]}</span>
						<span className="block text-primary">{t.title[1]}</span>
					</h2>
					<p
						className={`mt-6 max-w-[60ch] text-lg leading-relaxed text-foreground-muted ${body}`}
					>
						{t.note}
					</p>
				</div>
				...
				<ul className="mt-10 flex flex-col gap-3">
					{t.promise.map((p) => (
						<li
							key={p}
							className={`flex items-center gap-3 text-base text-foreground ${display}`}
						>
