			...
			<div className="layout-shell relative z-10">
				<header className="grid gap-8 lg:grid-cols-[1fr_1.2fr] lg:gap-16 lg:items-end">
					...
					<div className="lg:pb-2">
						...
						<p className={`mt-3 max-w-[60ch] text-base leading-relaxed text-foreground-subtle ${body}`}>
							{t.lead}
						</p>
						<div className="mt-6 flex items-center gap-3">
							<span className={`mkt-rule-short ${mono}`} />
							<span className={`text-[11px] font-medium uppercase tracking-[0.22em] text-foreground-muted ${display}`}>
					...
					{t.rows.map((row) => (
						...
						>
							...
							<div>
								...
								<p
									className={`mt-2 text-base font-medium text-foreground-muted ${display}`}
								>
									{row.lede}
								</p>
							</div>
							<div>
								<p
									className={`text-base leading-relaxed text-foreground-muted ${body}`}
								>
									{row.body}
								</p>
								<div
									className={`mt-4 border-l-2 border-primary/40 pl-4 ${body}`}
								>
