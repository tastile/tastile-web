			...
			<div className="layout-shell relative z-10">
				...
				<div className="mt-16 grid gap-12 lg:grid-cols-[12rem_1fr] lg:gap-20">
					<div className="hidden lg:block">
						<p
							aria-hidden
							className="mkt-faq-marker select-none"
						>
							?
						</p>
					...
					<ol className="mkt-timeline relative">
						{t.items.map((item, i) => (
							<li
								key={item.q}
								className="relative border-b border-surface-2 py-7 first:border-t"
							>
									...
									<div>
										<details className="mkt-details group">
											<summary className="flex items-start justify-between gap-6 text-left">
												<span className={`text-lg font-semibold text-foreground lg:text-xl ${display}`}>
													{item.q}
												</span>
											</summary>
											<p className={`mt-4 max-w-[64ch] text-base leading-relaxed text-foreground-muted ${body}`}>
												{item.a}
											</p>
										</details>
									</div>
