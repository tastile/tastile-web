...
export function Manifesto({ t, lang }: { t: Dict["manifesto"]; lang: Lang }) {
	...
	return (
		<section className="relative overflow-hidden py-24 lg:py-32">
			...
			<div
				aria-hidden
				className="pointer-events-none absolute top-12 left-[-2rem] z-0 select-none"
			>
			...
			<div className="layout-shell relative z-10">
				<header className="max-w-3xl">
					...
					<h2
						className={`mt-3 mkt-display-2 text-foreground ${display}`}
					>
				...
				<div className="mt-16">
					...
					<div className="grid gap-6 border-b border-surface-2 pb-12 lg:grid-cols-[10rem_1fr] lg:gap-12">
						...
						<div>
							<p className={`text-[11px] font-medium uppercase tracking-[0.22em] text-foreground-subtle ${display}`}>
								{t.leftLabel}
							</p>
								...
								{t.leftItems.map((item) => (
									...
									>
										<Circle
											...
											strokeWidth={1.5}
										/>
										<span className="line-through decoration-foreground-subtle/40">
											{item}
										</span>
									</li>
								))}
					...
					<div className="grid gap-6 pt-12 lg:grid-cols-[10rem_1fr] lg:gap-12">
						<div className="flex items-start gap-3">
							<span className={`${mono} text-sm text-primary tabular-nums`}>
								新
							</span>
						...
						<div>
							...
							<p className={`mt-6 mkt-pullquote text-foreground ${display}`}>
								{t.rightHeadline}
							</p>
							<p className={`mt-3 max-w-[60ch] text-base leading-relaxed text-foreground-muted ${body}`}>
								{t.rightSubtext}
							</p>
				...
				<div className="mt-24 grid gap-8 lg:grid-cols-[1fr_2fr] lg:gap-16">
					<div>
						<h3 className={`mkt-display-2 text-foreground ${display}`}>
							{t.timelineTitle}
						</h3>
						<p className={`mt-4 max-w-[40ch] text-base leading-relaxed text-foreground-muted ${body}`}>
							{t.timelineSubtitle}
						</p>
					</div>
						...
						{t.timeline.map((event) => (
							...
							>
								<div className="flex flex-col items-start gap-1">
									...
									<span className={`${mono} text-[10px] uppercase tracking-[0.18em] ${kindColor[event.kind] ?? "text-foreground-muted"}`}>
