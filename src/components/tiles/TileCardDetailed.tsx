...
export function TileCardDetailed(props: TileCardDetailedProps) {
  ...
  return (
    <div className={cn(TILE_CARD_STYLES.base, TILE_CARD_STYLES.padding.detailed, "space-y-4")}>
      {/* Header */}
      <div className="flex items-start gap-3">
        <TileStatusIcon lifecycle={lifecycle} onClick={handleStatusClick} size={24} />
        ...
        <div className="flex-1 min-w-0">
          ...
          {tile.core.nextAction && (
            <p className="mt-1 text-sm text-foreground-muted">{tile.core.nextAction}</p>
          )}
        </div>
      ...
      {/* Action Buttons */}
      <div className="pt-2">
        <TileActionButtons tile={tile} variant="full" {...actions} />
      </div>
    </div>
  );
}
