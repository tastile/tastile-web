...
export default function Events() {
	...
	const load = useCallback(() => {
		...
		return getCoreClient()
			.call<{ events: DebugEvent[]; count: number } | DebugEvent[]>(
				"getDebugEvents",
			)
			.then((result) => {
				setEvents(result);
				...
				sections={[
					{
						...
						items: [
							{
								label: t("dashboard.events.labels.loaded"),
								value: list.length,
							},
							{
								label: t("dashboard.events.labels.distinctTypes"),
					...
					},
					{
						...
						items: [
							{
								label: t("dashboard.events.labels.timeline"),
								value: "→",
								href: "/dashboard/timeline",
							},
							{
								label: t("dashboard.events.labels.runtime"),
								value: "→",
								href: "/dashboard/runtime",
							},
							{
								label: t("dashboard.events.labels.apiExplorer"),
								value: "→",
								href: "/dashboard/api",
							},
						],
						...
						<thead className="bg-surface-0 text-caption font-semibold uppercase tracking-wider text-ink-3">
							<tr>
								...
								<th className="w-20 px-2 py-2">
									{t("dashboard.events.table.type")}
								</th>
								<th className="px-2 py-2">{t("dashboard.events.table.id")}</th>
								<th className="px-2 py-2">
									{t("dashboard.events.table.actor")}
								</th>
								<th className="hidden px-2 py-2 md:table-cell">
									{t("dashboard.events.table.tile")}
								</th>
								<th className="px-2 py-2 text-right">
									{t("dashboard.events.table.when")}
								</th>
							</tr>
						</thead>
						<tbody>
	...
	return (
		<>
			<tr
				...
				tabIndex={0}
				role="button"
				aria-label={`${event.type} ${event.id}`}
				...
				<tr className="bg-surface-0">
					<td colSpan={6} className="px-4 py-3">
						<div className="mb-2 flex items-center gap-2 text-caption font-semibold uppercase tracking-wider text-ink-3">
							...
							{t("dashboard.events.payloadHeading")}
						</div>
						...
						<div className="mt-2 grid grid-cols-2 gap-2 text-caption sm:grid-cols-4">
							<Field
								label={t("dashboard.events.detail.eventId")}
								value={event.id}
							/>
							<Field
								label={t("dashboard.events.table.occurred")}
								value={event.occurred_at}
							/>
							<Field
								label={t("dashboard.events.table.tile")}
								value={event.tile_id ?? "—"}
							/>
							<Field
...
function Field({ label, value }: { label: string; value: string }) {
	return (
		<div className="rounded-md border border-border bg-surface-1 p-2">
			<div className="text-caption font-semibold uppercase tracking-wider text-ink-4">
				{label}
			</div>
