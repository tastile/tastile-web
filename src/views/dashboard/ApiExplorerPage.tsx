				...
				sections={[
					{
						...
						items: [
							{
								label: t("dashboard.api.labels.endpoints"),
								value: Object.keys(ENDPOINTS).length,
							},
							{
								label: t("dashboard.api.labels.tags"),
								value: TAG_ORDER.length,
							},
					...
					},
					{
						...
						items: [
							{
								label: t("dashboard.api.labels.runtime"),
								value: "→",
								href: "/dashboard/runtime",
							},
							{
								label: t("dashboard.api.labels.eventsLog"),
								value: "→",
								href: "/dashboard/events",
							},
							{
								label: t("dashboard.api.labels.quota"),
								value: "→",
				...
				meta={
					<>
						...
						<Badge variant="light" color="gray" size="sm" radius="xl">
							{t("dashboard.api.endpointsLabel", {
				...
				<div className="flex flex-wrap items-center gap-1.5">
					...
					{TAG_ORDER.map((tag) => (
						<Chip
							...
							radius="sm"
						>
							{t("dashboard.api.tagFilterWithCount", {
								tag,
								count: ENDPOINTS_BY_TAG[tag].length,
							})}
						</Chip>
					))}
				</div>
					...
					<tbody>
						{keys.map((k) => {
							...
							return (
								<tr
									...
									tabIndex={0}
									role="button"
									aria-label={`${m.method} ${m.path} — ${m.summary}`}
									className={cn(
								...
								>
									...
									<td className="px-3 py-1.5">
										<code className="font-mono text-xs text-ink-1">
											{m.path}
										</code>
									</td>
									<td className="truncate px-3 py-1.5 text-ink-2">
										{m.summary}
									</td>
						...
						{keys.length === 0 ? (
							<tr>
								<td
									colSpan={6}
									className="px-3 py-12 text-center text-sm text-ink-4"
								>
				...
				<div className="bg-surface-0 p-4">
					...
					{meta.method !== "GET" ? (
						<div className="mt-4">
							<div className="mb-1.5 flex items-center justify-between">
								...
								<Button
									onClick={() => setBodyText(defaultBody(endpointKey, t))}
								>
							...
							<Textarea
								...
								spellCheck={false}
								h={176}
								aria-label={t("dashboard.api.bodyLabel")}
								styles={{
									input: {
					...
					<div className="mt-4 flex items-center gap-2">
						...
						<Button
							...
							size="sm"
							onClick={() =>
								copyToClipboard(curlCommand(meta.method, meta.path, bodyText))
							}
							leftSection={<Copy size={14} />}
						>
							{t("dashboard.api.copyAsCurl")}
						</Button>
						<span className="ml-auto font-mono text-caption text-ink-4">
							{endpointKey}
						</span>
					</div>
				</div>
...
function defaultBody(k: EndpointKey, t: (key: string) => string): string {
	switch (k) {
		case "createTile":
		...
		case "startBreak":
			return JSON.stringify(
				{ linked_tile_id: null, break_min: 5, reason: null },
				null,
				2,
			);
		case "endBreak":
			return JSON.stringify(
				{ tile_id: null, ended_at: new Date().toISOString() },
				null,
				2,
			);
		case "requestPrompt":
		...
		case "attachMemo":
			return JSON.stringify(
				{
					tile_id: "00000000-0000-0000-0000-000000000000",
					body: t("dashboard.api.placeholderMemoBody"),
				},
				null,
				2,
			);
...
async function executeApiCall(
	...
	const query = inputs.queryText.trim()
		? (JSON.parse(inputs.queryText) as Record<
				string,
				string | number | boolean
			>)
		: undefined;
	const body =
		method !== "GET" && inputs.bodyText.trim()
			? JSON.parse(inputs.bodyText)
			: undefined;
