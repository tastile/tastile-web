...
- Japanese source-code comments in `src/execution/model/{domain-reducer,validate}.ts`, `src/shared/stores/quick-create-store.ts`, `src/features/create-tile/ui/SourceGenerationPanel.tsx`, and `src/shared/api/v1/decision.ts` were translated to English. Technical design-doc references that mention Japanese proper nouns (V1_015, 休憩 seed, C-recurring-source.md) are kept verbatim because they are spec traceability anchors, not English-comment violations.
- The five-entry blanket ESLint ignore (`src/test`, `src/shared`, `src/views`, `src/widgets`, `src/features`) is removed. With Biome's project-wide linter disabled, lint now actually runs over production src/**. Re-enabling surfaced exactly one stale `// eslint-disable-next-line react-hooks/exhaustive-deps` in QuickCreate.tsx (the rule's plugin is not in package.json — the disable was always a no-op), and that line was deleted. Subsequent `bun run lint` reports zero errors and zero warnings over the full tree.
- Remaining follow-up: audit each previously-ignored tree one slice at a time once feature-sliced rename work settles, to confirm no further stale `eslint-disable-*` annotations remain anywhere in src/. Track in a follow-up ADR when the rename refactor lands.
