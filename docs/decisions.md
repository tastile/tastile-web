...
## 2026-08-12 i18n and ESLint config debt

- `src/features/create-tile/ui/BehaviorPreview.tsx` no longer carries hardcoded Japanese literals; all 18 keys (`behaviorPreviewScheduleLabel`, `behaviorPreviewExecutionTimeLabel`, `behaviorPreviewRecurrenceLabel`, `behaviorPreviewWindowLabel`, `behaviorPreviewPlacementLabel`, `behaviorPreviewObjectiveLabel`, `behaviorPreviewReferenceLabel`, `behaviorPreviewPriorityPrefix`, `behaviorPreviewSplitAllow`, `behaviorPreviewSplitKeep`, `behaviorPreviewRepeatInterval`, `behaviorPreviewRepeatIntervalUnit{Min,Hour,Day}`, `behaviorPreviewRecurrenceKind{Daily,Weekly,Interval}`, `behaviorPreviewUntitledFallback`) live in `src/shared/i18n/sections/features/quickcreate-source.ts` under all five locales (ja / en / zh-CN / ko / es). Sub-components (Duration/Repeat/Source/Relations/Tasks Preview) take a `t` prop so the parent passes the translation function through.
- Japanese source-code comments in `src/execution/model/{domain-reducer,validate}.ts`, `src/shared/stores/quick-create-store.ts`, `src/features/create-tile/ui/SourceGenerationPanel.tsx`, and `src/shared/api/v1/decision.ts` were translated to English. Technical design-doc references that mention Japanese proper nouns (V1_015, 休憩 seed, C-recurring-source.md) were kept verbatim because they are spec traceability anchors, not English-comment violations.
- Remaining debt: `eslint.config.mts` still ignores five production source directories wholesale (`src/test`, `src/shared`, `src/views`, `src/widgets`, `src/features`), so new ESLint violations in those trees are silently dropped. Intentional narrow fixes were applied for the `src/components/**` dead entry. Future cleanup: re-enable lint per file, then narrow-exclude only the legitimate offenders (likely ~5-10 specific files vs the current ~1000-file blanket). Tracked as a follow-up ADR when ESLint reactivation begins.
- `playwright.config.ts` had its `webServer.env` indentation sanitized by commit `ca7dfde` (audit issue #96), so no further cosmetic work is needed.
